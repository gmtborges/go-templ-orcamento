...
func testBiddingsExists(t *testing.T) {
	...
	e, err := BiddingExists(ctx, tx, o.ID)
	if err != nil {
...
func testBiddingsFind(t *testing.T) {
	...
	biddingFound, err := FindBidding(ctx, tx, o.ID)
	if err != nil {
		t.Error(err)
	}
...
func testBiddingToManyAutoOffers(t *testing.T) {
	...
	for _, v := range check {
		if v.BiddingID == b.BiddingID {
			bFound = true
		}
		if v.BiddingID == c.BiddingID {
			cFound = true
		}
	}
...
func testBiddingToManyAddOpAutoOffers(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.ID != first.BiddingID {
			t.Error("foreign key was wrong value", a.ID, first.BiddingID)
		}
		if a.ID != second.BiddingID {
			t.Error("foreign key was wrong value", a.ID, second.BiddingID)
		}
...
func testBiddingToOneAutoCategoryUsingCategory(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var local Bidding
	var foreign AutoCategory

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, biddingDBTypes, false, biddingColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Bidding struct: %s", err)
	}
	if err := randomize.Struct(seed, &foreign, autoCategoryDBTypes, false, autoCategoryColumnsWithDefault...); err != nil {
	...
	AddAutoCategoryHook(boil.AfterSelectHook, func(ctx context.Context, e boil.ContextExecutor, o *AutoCategory) error {
...
func testBiddingToOneSetOpAutoCategoryUsingCategory(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, autoCategoryDBTypes, false, strmangle.SetComplement(autoCategoryPrimaryKeyColumns, autoCategoryColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, autoCategoryDBTypes, false, strmangle.SetComplement(autoCategoryPrimaryKeyColumns, autoCategoryColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*AutoCategory{&b, &c} {
...
var (
	biddingDBTypes = map[string]string{`ID`: `integer`, `CompanyID`: `integer`, `Title`: `character varying`, `Description`: `text`, `StartDate`: `timestamp without time zone`, `EndDate`: `timestamp without time zone`, `CategoryID`: `integer`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)
