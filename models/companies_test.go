...
func testCompaniesExists(t *testing.T) {
	...
	e, err := CompanyExists(ctx, tx, o.CompanyID)
	if err != nil {
...
func testCompaniesFind(t *testing.T) {
	...
	companyFound, err := FindCompany(ctx, tx, o.CompanyID)
	if err != nil {
		t.Error(err)
	}
...
func testCompanyToManyBiddings(t *testing.T) {
	...
	queries.Assign(&b.CompanyID, a.CompanyID)
	queries.Assign(&c.CompanyID, a.CompanyID)
...
func testCompanyToManyAddOpBiddings(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if !queries.Equal(a.CompanyID, first.CompanyID) {
			t.Error("foreign key was wrong value", a.CompanyID, first.CompanyID)
		}
		if !queries.Equal(a.CompanyID, second.CompanyID) {
...
var (
	companyDBTypes = map[string]string{`CompanyID`: `INTEGER`, `Name`: `TEXT`, `Address`: `TEXT`, `ContactInfo`: `TEXT`, `CreatedAt`: `DATETIME`, `UpdatedAt`: `DATETIME`}
	_              = bytes.MinRead
)
...
func testCompaniesSliceUpdateAll(t *testing.T) {
	...
	if strmangle.StringSliceMatch(companyAllColumns, companyPrimaryKeyColumns) {
		...
		fields = strmangle.SetComplement(fields, companyGeneratedColumns)
	}
