...
func (roleL) LoadUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("\"users\".\"id\", \"users\".\"email\", \"users\".\"password\", \"users\".\"name\", \"users\".\"company_id\", \"users\".\"created_at\", \"users\".\"updated_at\", \"a\".\"role_id\""),
		qm.From("\"users\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Email, &one.Password, &one.Name, &one.CompanyID, &one.CreatedAt, &one.UpdatedAt, &localJoinCol)
		if err != nil {
