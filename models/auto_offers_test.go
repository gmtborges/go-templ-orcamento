...
func testAutoOffersExists(t *testing.T) {
	...
	e, err := AutoOfferExists(ctx, tx, o.ID)
	if err != nil {
...
func testAutoOffersFind(t *testing.T) {
	...
	autoOfferFound, err := FindAutoOffer(ctx, tx, o.ID)
	if err != nil {
		t.Error(err)
	}
...
func testAutoOfferToOneAutoStoreUsingAutoStore(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err := randomize.Struct(seed, &foreign, autoStoreDBTypes, false, autoStoreColumnsWithDefault...); err != nil {
	...
	local.AutoStoreID = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.AutoStore().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadAutoStore(ctx, tx, false, (*[]*AutoOffer)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.AutoStore == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.AutoStore = nil
	if err = local.L.LoadAutoStore(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.AutoStore == nil {
		t.Error("struct should have been eager loaded")
...
func testAutoOfferToOneBiddingUsingBidding(t *testing.T) {
	...
	if err := randomize.Struct(seed, &foreign, biddingDBTypes, false, biddingColumnsWithDefault...); err != nil {
...
func testAutoOfferToOneSetOpAutoStoreUsingAutoStore(t *testing.T) {
	var err error
	...
	for i, x := range []*AutoStore{&b, &c} {
		err = a.SetAutoStore(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.AutoStoreID != x.ID {
			t.Error("foreign key was wrong value", a.AutoStoreID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.AutoStoreID))
		reflect.Indirect(reflect.ValueOf(&a.AutoStoreID)).Set(zero)
		...
		if a.AutoStoreID != x.ID {
			t.Error("foreign key was wrong value", a.AutoStoreID, x.ID)
		}
	}
}
func testAutoOfferToOneSetOpBiddingUsingBidding(t *testing.T) {
	...
	for i, x := range []*Bidding{&b, &c} {
		...
		if a.BiddingID != x.ID {
			t.Error("foreign key was wrong value", a.BiddingID, x.ID)
		}
	}
}
...
var (
	autoOfferDBTypes = map[string]string{`ID`: `integer`, `BiddingID`: `integer`, `AutoStoreID`: `integer`, `OfferDetails`: `text`, `OfferDate`: `timestamp without time zone`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_                = bytes.MinRead
)
